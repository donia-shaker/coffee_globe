...
const props = defineProps({
    langs: Array,
    contact_us_info: Object, // <-- تم التعديل هنا
});

const contact_us_info = props.contact_us_info;
...
props.langs.forEach((lang) => {
    initialForm[`name_${lang.code}`] =
        contact_us_info.name?.[lang.code] || "";
    initialForm[`value_${lang.code}`] = contact_us_info.value?.[lang.code] || "";
});
...
const submit = () => {
    form.post(route("contact_us_infos.update", contact_us_info.id), {
        forceFormData: true,
    });
};
</script>
...
<template>
    <DashboardLayout>
        <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-2 mb-3 fs-2">تعديل وسيلة التواصل</h4>
