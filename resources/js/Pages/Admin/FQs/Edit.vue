...
const props = defineProps({
    langs: Array,
    fq: Object, // <-- تم التعديل هنا
});

const fq = props.fq;

const initialForm = {
    is_active: fq.is_active ? "1" : "0",
    _method: "put",
};

// تعبئة أسماء اللغات
props.langs.forEach((lang) => {
    initialForm[`name_${lang.code}`] = fq.name?.[lang.code] || "";
    initialForm[`text_${lang.code}`] = fq.text?.[lang.code] || "";
});
...
const submit = () => {
    form.post(route("fqs.update", fq.id), {
        forceFormData: true,
        onSuccess: () => {},
    });
};
...
<template>
    <DashboardLayout>
        <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-2 mb-3 fs-2">تعديل الأسئلة الشائعة</h4>
