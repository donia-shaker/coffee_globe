...
namespace App\Http\Controllers\Admin\Value;

use App\Http\Controllers\Controller;
use App\Http\Requests\Admin\Value\ValueRequest;
...
class ValueController extends Controller
{
     public function index(Request $request)
    {
        ...
        if (Schema::hasColumn((new Value)->getTable(), $sort)) {
        ...
        return Inertia::render('Admin/Values/Index', [
            'values' => $query->paginate($perPage), // استخدم $perPage هنا
    ...
    public function store(ValueRequest $request)
    {
        ...
        try {
            ...
            return to_route('values.index')->with('success', 'تمت الاضافة بنجاح');
        } catch (\Exception $e) {
            return to_route('values.index')->with('error', 'Something went wrong :(');
        }
    }
    ...
    public function update(ValueRequest $request, $id)
    {
        ...
        if ($request->hasFile('image') && $request->file('image')->isValid()) {
            if (isset($value->media)) {
                Media::where('model', 'value')->where('id', $value->media->id)->delete();
            }
            // upload image
            $this->media_controller->saveImage('value', $value->id, $request->file('image'));
        }
